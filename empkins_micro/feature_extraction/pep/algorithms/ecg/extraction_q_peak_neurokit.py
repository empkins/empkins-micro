...
class QPeakExtractionNeurokit(BaseExtraction):
    ...
    def extract(self, signal: pd.DataFrame, heartbeats: pd.DataFrame, sampling_rate_hz: int):
        """function which extracts Q-wave peaks from given ECG signal
        ...
        _, waves = nk.ecg_delineate(signal, rpeaks=r_peaks, sampling_rate=sampling_rate_hz, method="dwt", show=True,
                                    show_type="peaks")  # show can also be set to False
