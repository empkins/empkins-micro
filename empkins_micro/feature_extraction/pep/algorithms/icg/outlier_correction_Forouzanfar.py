...
class OutlierCorrectionForouzanfar(BaseExtraction):
    ...
    @make_action_safe
    def extract(
        self, b_points: pd.DataFrame, c_points: pd.DataFrame, sampling_rate_hz: int
    ):
    ...
    def stationarize_data(
        b_points: pd.DataFrame, c_points: pd.DataFrame, sampling_rate_hz: int
    ) -> pd.DataFrame:
        dist_to_C = (
            (c_points["c_point"] - b_points["b_point"]) / sampling_rate_hz
        ).to_frame()
    ...
    def detect_outliers(stationary_data: pd.DataFrame) -> pd.DataFrame:
        ...
        median_abs_dev = median_abs_deviation(
            stationary_data["statio_data"], axis=0, nan_policy="propagate"
        )
        outliers = pd.DataFrame(index=stationary_data.index, columns=["outliers"])
        outliers["outliers"] = False
        outliers.loc[
            (stationary_data["statio_data"] - median) > (3 * median_abs_dev), "outliers"
        ] = True
    ...
    def correct_outliers(
        ...
        for idx in outliers.index:
            ...
            backward_prediction = b_arima_res.predict(
                len(reversed_input) - idx, len(reversed_input) - idx
            )
